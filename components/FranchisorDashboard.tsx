...
export default function FranchisorDashboard() {
      ...
      <div className="p-8">
        <div className="mb-6">
          <button
            ...
            className="text-[#AC1305] hover:underline mb-2"
          >
            ← Back to All Locations
          </button>
          <h1 className="text-3xl font-bold text-gray-900">{locationData.locationName} - Detailed View</h1>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-white p-6 rounded-lg shadow">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-gray-600">MTD Revenue</span>
              <DollarSign className="text-green-600" size={20} />
            </div>
            <div className="text-2xl font-bold text-gray-900">{formatCurrency(locationData.revenue.mtd)}</div>
            <div className={`text-sm mt-1 ${locationData.revenue.momGrowth >= 0 ? 'text-green-600' : 'text-red-600'}`}>
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="text-2xl font-bold text-gray-900">{locationData.members.active}</div>
            <div className={`text-sm mt-1 ${locationData.members.netGrowth >= 0 ? 'text-green-600' : 'text-red-600'}`}>
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="text-2xl font-bold text-gray-900">{locationData.leads.conversionRate.toFixed(1)}%</div>
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-gray-600">Class Fill Rate</span>
              <Calendar className="text-orange-600" size={20} />
            </div>
            <div className="text-2xl font-bold text-gray-900">{locationData.classes.averageFillRate.toFixed(1)}%</div>
            <div className="text-sm text-gray-600 mt-1">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              ...
              <div className="text-lg font-semibold text-gray-900">{formatCurrency(locationData.revenue.ytd)}</div>
            </div>
            <div>
              <div className="text-sm text-gray-600">YoY Growth</div>
              <div className={`text-lg font-semibold ${locationData.revenue.yoyGrowth >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                {formatPercent(locationData.revenue.yoyGrowth)}
              </div>
            </div>
            <div>
              <div className="text-sm text-gray-600">Retention Rate</div>
              <div className="text-lg font-semibold text-gray-900">{locationData.members.retention.toFixed(1)}%</div>
            </div>
            <div>
              <div className="text-sm text-gray-600">Churn Rate</div>
              <div className="text-lg font-semibold text-red-600">{locationData.members.churn.toFixed(1)}%</div>
            </div>
            <div>
              <div className="text-sm text-gray-600">New Members MTD</div>
              <div className="text-lg font-semibold text-green-600">+{locationData.members.new}</div>
            </div>
            <div>
              <div className="text-sm text-gray-600">Cancelled MTD</div>
              <div className="text-lg font-semibold text-red-600">-{locationData.members.cancelled}</div>
            </div>
            <div>
              <div className="text-sm text-gray-600">Total Staff</div>
              <div className="text-lg font-semibold text-gray-900">{locationData.staff.total}</div>
            </div>
            <div>
              <div className="text-sm text-gray-600">Total Classes</div>
              <div className="text-lg font-semibold text-gray-900">{locationData.classes.total}</div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Franchisor Dashboard</h1>
        <p className="text-gray-600">Monitor performance across all franchise locations</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
        <div className="bg-gradient-to-br from-green-500 to-green-600 p-6 rounded-lg shadow-lg text-white">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm opacity-90">Total Revenue MTD</span>
            <DollarSign size={24} />
          </div>
          <div className="text-3xl font-bold">{formatCurrency(overview.totalRevenue.mtd)}</div>
        ...
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 p-6 rounded-lg shadow-lg text-white">
          ...
          <div className="text-3xl font-bold">{overview.totalMembers.active}</div>
          <div className="flex items-center mt-2 text-sm">
        ...
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 p-6 rounded-lg shadow-lg text-white">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm opacity-90">Lead Conversion</span>
            <Target size={24} />
          </div>
          <div className="text-3xl font-bold">{overview.totalLeads.conversionRate.toFixed(1)}%</div>
          <div className="text-sm mt-2 opacity-90">{overview.totalLeads.count} total leads</div>
        </div>

        <div className="bg-gradient-to-br from-orange-500 to-orange-600 p-6 rounded-lg shadow-lg text-white">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm opacity-90">Avg Fill Rate</span>
            <Calendar size={24} />
          </div>
          <div className="text-3xl font-bold">{overview.averageFillRate.toFixed(1)}%</div>
          <div className="text-sm mt-2 opacity-90">Across all locations</div>
        </div>

        <div className="bg-gradient-to-br from-red-500 to-red-600 p-6 rounded-lg shadow-lg text-white">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm opacity-90">Churn Rate</span>
            <TrendingDown size={24} />
          </div>
          <div className="text-3xl font-bold">{overview.totalMembers.churn.toFixed(1)}%</div>
          <div className="text-sm mt-2 opacity-90">Month to date</div>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow mb-8">
        <h2 className="text-xl font-bold text-gray-900 mb-4">Member Metrics by Location</h2>
        <ResponsiveContainer width="100%" height={300}>
      ...
      <div className="bg-white p-6 rounded-lg shadow mb-8">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-gray-900">Location Performance Rankings</h2>
          <div className="flex gap-2">
            <button
              onClick={() => setSelectedMetric('revenue')}
              className={`px-3 py-1 rounded text-sm ${selectedMetric === 'revenue' ? 'bg-[#AC1305] text-white' : 'bg-gray-200 text-gray-700'}`}
            >
              Revenue
            </button>
            <button
              onClick={() => setSelectedMetric('members')}
              className={`px-3 py-1 rounded text-sm ${selectedMetric === 'members' ? 'bg-[#AC1305] text-white' : 'bg-gray-200 text-gray-700'}`}
            >
              Members
            </button>
            <button
              onClick={() => setSelectedMetric('conversion')}
              className={`px-3 py-1 rounded text-sm ${selectedMetric === 'conversion' ? 'bg-[#AC1305] text-white' : 'bg-gray-200 text-gray-700'}`}
            >
              Conversion
            </button>
            <button
              onClick={() => setSelectedMetric('fillRate')}
              className={`px-3 py-1 rounded text-sm ${selectedMetric === 'fillRate' ? 'bg-[#AC1305] text-white' : 'bg-gray-200 text-gray-700'}`}
            >
              Fill Rate
            </button>
          </div>
      ...
      {/* Royalty Calculator */}
      <div className="bg-white p-6 rounded-lg shadow mb-8">
        <h2 className="text-xl font-bold text-gray-900 mb-4">Royalty & Brand Fund Calculator</h2>
        <div className="overflow-x-auto">
          <table className="min-w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Brand Fund (2%)</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {royalties.map((r) => (
                <tr key={r.location}>
                  <td className="px-4 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{r.location}</td>
                  <td className="px-4 py-4 whitespace-nowrap text-right text-sm text-gray-900">
                    {formatCurrency(overview.locationMetrics.find(l => l.locationName === r.location)?.revenue.mtd || 0)}
                  </td>
                  <td className="px-4 py-4 whitespace-nowrap text-right text-sm text-gray-900">{formatCurrency(r.royalty)}</td>
                  <td className="px-4 py-4 whitespace-nowrap text-right text-sm text-gray-900">{formatCurrency(r.brandFund)}</td>
                  <td className="px-4 py-4 whitespace-nowrap text-right text-sm font-semibold text-gray-900">{formatCurrency(r.total)}</td>
                </tr>
              ))}
        ...
        <div className="mt-4">
          <button className="bg-[#AC1305] text-white px-4 py-2 rounded hover:bg-[#8B0F04]">
            Export to CSV
          </button>
        </div>
      </div>
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
        <h2 className="text-lg font-bold text-yellow-900 mb-3">⚠️ Alerts & Insights</h2>
        <ul className="space-y-2">
          ...
          {overview.locationMetrics.every(loc => loc.members.churn <= 5 && loc.leads.conversionRate >= 20 && loc.classes.averageFillRate >= 60) && (
            <li className="text-sm text-green-800">✓ All locations performing within healthy ranges</li>
          )}
        </ul>
      </div>
    </div>
